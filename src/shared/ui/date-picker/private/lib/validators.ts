...
export const isPreviousMonth = (date: Date | number): boolean => {
  const today = new Date();
  const { month: prevMonth, year: prevYear } = getPreviousMonth(today.getMonth(), today.getFullYear());
...
export const isDateInRange = (date: Date | number, startDate: Date | number, endDate: Date | number): boolean => {
...
export const isSelectedDayInPreviousMonth = (date: Date | number, month: number, year: number): boolean => {
  const inputDate = toDate(date);
...
export const isSelectedDayInNextMonth = (date: Date | number, month: number, year: number): boolean => {
  const inputDate = toDate(date);
