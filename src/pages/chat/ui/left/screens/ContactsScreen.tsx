...
const ContactsScreen = forwardRef<HTMLDivElement, OwnProps & StateProps>((props, ref) => {
  ...
  const handlePrevClick = useLastCallback(() => {
    setScreen(LeftColumnScreenType.Main);
  });

  const handleSortTypeChange = useLastCallback((type: string) => {
    setSortType(type);
  });

  return (
    <div ref={ref} className={buildClassName(className, s.ContactScreen)}>
      <HeaderNavigation className={s.ContactsHeader} name="Contacts" onPrevClick={handlePrevClick}>
        <IconButton>
          <SearchRounded />
        </IconButton>
      ...
      <section className={buildClassName(s.ContactsCreateActions, s.ContactSection)}>
        <ActionButton icon={<PersonAddAlt1Rounded />}>Add contact</ActionButton>
        <ActionButton icon={<Groups2Rounded />}>Create group</ActionButton>
        <ActionButton icon={<ConnectWithoutContactRounded />}>Create channel</ActionButton>
        <ActionButton icon={<ReduceCapacityRounded />}>Create own server</ActionButton>
      </section>
      ...
      <section className={buildClassName(s.ContactsResultList, s.ContactSection)}>
