...
const getPixelEnergy = (
  ...
  return mA > ALPHA_DELETE_THRESHOLD
    ? lEnergy + rEnergy
    : getPixelDeleteEnergy();
};

const getPixelEnergyH = (
  img: ImageData,
  { w }: ImageSize,
  { x, y }: Coordinate,
): number => {
...
const reCalculateEnergyMapH = (
  ...
  seam.forEach(({ x: seamX, y: seamY }: Coordinate) => {
    ...
    energyMap[seamY][seamX] = getPixelEnergyH(
      img,
      { w, h },
      { x: seamX, y: seamY },
    );
...
const reCalculateEnergyMapV = (
  ...
  seam.forEach(({ x: seamX, y: seamY }: Coordinate) => {
    ...
    // Recalculating the energy pixels around the deleted seam.
    energyMap[seamY][seamX] = getPixelEnergyV(
      img,
      { w, h },
      { x: seamX, y: seamY },
    );
...
/**
 * Resizes an image to a specific width and height using the seam carving technique.
 * This function removes seams (horizontal or vertical) with the lowest energy.
 * The resizing process is performed in two stages: first adjusting the width,
 * and then adjusting the height.
 *
 * @param {ResizeImageArgs} args - The arguments required to resize the image.
 * @param {ImageData} args.img - The image data to resize.
 * @param {number} args.toWidth - The target width of the resized image.
 * @param {number} args.toHeight - The target height of the resized image.
 * @param {(args: OnIterationArgs) => void} [args.onIteration] - A callback function invoked on each iteration.
 * It provides information about the current seam, energy map, image data, and progress.
 *
 * @returns {Promise<void>} A promise that resolves when the image has been resized.
 */
export const resizeImage = async ({
  ...
  const onResizeIteration = async (
    onIterationArgs: OnIterationArgs,
  ): Promise<void> => {
    const {
      seam,
      img: onIterationImg,
